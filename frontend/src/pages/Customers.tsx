...
export default function Customers() {
  ...
  const [form, setForm] = useState({ name: '', email: '', language: 'cs', contact_person: '', notify_email: true })
  ...
  return (
    <div className="p-8">
      ...
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <input className="input" placeholder="Název *" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
            <input className="input" placeholder="E-mail *" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} />
            <input className="input" placeholder="Kontaktní osoba" value={form.contact_person} onChange={e => setForm({ ...form, contact_person: e.target.value })} />
          ...
          <div className="flex gap-2 mt-4">
            <button onClick={() => create.mutate(form)}
              className="bg-brand text-white px-4 py-2 rounded-lg text-sm hover:bg-blue-800">
              Uložit
            </button>
      ...
      <div className="space-y-4">
          ...
          <div key={customer.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-sm text-gray-500">{customer.email} · {customer.language === 'cs' ? 'Čeština' : 'English'}</p>
                {customer.contact_person && <p className="text-xs text-gray-400 mt-0.5">Kontakt: {customer.contact_person}</p>}
              </div>
        ...
        {customers.length === 0 && <p className="text-gray-400 text-sm">Žádní zákazníci. Přidejte prvního.</p>}
      </div>
    </div>
  )
}
